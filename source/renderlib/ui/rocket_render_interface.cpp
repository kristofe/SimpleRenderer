...
namespace ui {
  ...
  _shaderTexture->registerShader("Shaders/librocketTexture.vert", ShaderType::VERTEX);
  _shaderTexture->registerShader("Shaders/librocketTexture.frag", ShaderType::FRAGMENT);
  _shaderTexture->compileShaders();
  _shaderTexture->linkShaders();

  _shaderNoTexture = new Shader();
  _shaderNoTexture->registerShader("Shaders/librocketNoTexture.vert", ShaderType::VERTEX);
  _shaderNoTexture->registerShader("Shaders/librocketNoTexture.frag", ShaderType::FRAGMENT);
  _shaderNoTexture->compileShaders();
  _shaderNoTexture->linkShaders();
  
  _mesh = new Mesh();
  //_mesh->createScreenQuad(Vector2(-1,-1), Vector2(1,1));
  //_mesh->createCube(1.0f);
  _mesh->cacheVertexAttributes(*_shader);
